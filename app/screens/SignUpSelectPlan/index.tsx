...
import SvgChefCooked from'app/svgs/SignUp/SvgChefCooked';
import SvgFresh from'app/svgs/SignUp/SvgFresh';
...
const SelectPlan = memo(() => {
  ...
  const onBoxSelect = useCallback((num) => {
    setBox(num);
  }, []);
  const onInfo = useCallback(() => {}, []);
  console.log('selected', selected);
  useEffect(() => {
    if (plan !== 0 && selected !== 0 && box !== 0) {
      setContinue(true);
  ...
  return (
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
        ...
        <View style={[styles.titleView, {marginTop: 24}]}>
          <Text style={styles.txtTitle}>Would you prefer a fresh or chef cooked box?</Text>
          <TouchableOpacity onPress={onInfo}>
            <SvgIconInfo />
          </TouchableOpacity>
        </View>
